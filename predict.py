...
def bright_image(image_path, brightness_factor=1.5, max_width=800, max_height=600):
    ...
    # im2 = lineRemoval.lines_removal(im)
    # cv2.imshow("resized_img",im2)
...
# C:\Users\SAI VISHWA\Downloads\word_segmentation-master (1)\word_segmentation-master\sgdo-115339r-50e-89887t-9977v.hdf5
...
def predict(filepath):
    ...
    for j in range(i):
        ...
        for _, x in enumerate(out):
            for p in x:
                if int(p) != -1:
                    recognized_text += char_list[int(p)]
